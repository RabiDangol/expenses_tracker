...
import {Form, Modal, Input, Button, Select, message, Table, DatePicker } from 'antd';
...
useEffect(() => {
    const getAllTransections =async () =>{
        try {
            ...
            setloading(true)
            const res =await axios.post('http://localhost:5000/api/v1/transection/get-transection',{
                userid:user._id,
                frequency,
                selectedDate
            })
...
const handleDelete =async(record) => {
    try {
        setloading(true)
        await axios.post('/transection/delete-transection',{transactionId:record._id})
        setloading(false)
    ...
    const handelsubmit = async (values)=>{
        try{
            ...
            if(editable){
                ...
                //             const user = JSON.parse(localStorage.getItem('user'));
                // await axios.get(`http://localhost:5000/api/v1/transection/get-transection?userid=${user._id}`);
...
return (
    <Layout>
        ...
        <div className="filters">
            <div> 
                {/* <img src="/cta-programming-img.webp" className='w-32'></img> */}
        ...
        footer={false}>
            ...
            <Form layout="vertical" onFinish={handelsubmit} initialValues={editable}> 
